...
python3 -m json.tool "$ROOT/web/packages/0.20.x/zh-CN/manifest.json" >/dev/null
python3 -m json.tool "$ROOT/web/packages/0.20.x/zh-CN/zh-cn.min.json" >/dev/null
...
compats = ("0.20.x", "0.16.x", "0.15.x", "0.14.x", "0.13.x", "legacy")
for idx, compat in enumerate(compats):
    ...
    # 中文包是 JSON 文本；在 Windows 克隆时 Git 可能将 LF 检出为 CRLF。
    # 发布清单固定校验仓库中的 LF 规范字节，避免平台差异造成伪失败。
    expected = hashlib.sha256(pkg.read_bytes().replace(b"\r\n", b"\n")).hexdigest()
    checks = [
...
if platforms["upstream"]["agent_version"] != "0.20.5":
    raise SystemExit("官方版本记录需为 0.20.5")
