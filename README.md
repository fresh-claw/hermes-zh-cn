...
Hermes 读取页面说明后，直接执行安装命令。安装器会在本机检测版本，失败或超时时使用 legacy 兼容包。安装后会把 Hermes 静态界面语言设置为 `zh`，同时加入小马AI中文增强说明和启动更新辅助脚本。
...
6. 安装器写入 `~/.hermes/skills/xiaoma-hermes-zh/SKILL.md`，让 Hermes 知道本项目的中文说明、边界和更新方式。
7. 可选启动更新由 `~/.xiaoma-hermes/bin/hermes` 包装命令完成：启动前检查 `latest.json`，有新版本就执行同一个安装器，再启动原 Hermes。
...
Some Hermes TUI messages still come from upstream code paths that are not exposed through the official language setting. Full TUI replacement requires a source patch or a Xiaoma-maintained build.
