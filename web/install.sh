...
download_script_from_urls() {
  ...
  for url in "$@"; do
    ...
    if curl_to_file "$url" "$out_file" && head -n 1 "$out_file" | grep -Eq '^#!.*(bash|sh)'; then
      if [ "$label" = "中文增强安装器" ] && ! grep -F "PACKAGE_VERSION=\"$PACKAGE_VERSION\"" "$out_file" >/dev/null 2>&1; then
        say "当前入口返回的中文增强安装器版本不匹配。"
        say "当前入口不可用或过慢，正在切换下一个入口。"
        continue
      fi
      if [ "$label" = "官方 Hermes 安装器" ] && ! grep -Eq 'hermes-agent|Hermes' "$out_file"; then
        say "当前入口返回的不是官方安装器脚本。"
        say "当前入口不可用或过慢，正在切换下一个入口。"
        continue
      fi
      printf '%s\n' "$url"
...
move_hermes_source_aside() {
  local stamp moved path target
  ...
  while IFS= read -r path; do
    ...
    if mv "$path" "$target" 2>/dev/null; then
      say "已备份异常官方 Hermes 源码目录：$target"
      moved=1
    else
      say "无法备份官方 Hermes 源码目录：$path"
    fi
  done <<EOF_HERMES_SOURCE_PATHS
$(hermes_source_paths)
EOF_HERMES_SOURCE_PATHS
  if [ "$moved" != "1" ]; then
    say "未找到可备份的官方 Hermes 源码目录。"
  fi
}

run_official_installer_with_repair() {
  local installer="$1"
  shift
  if bash "$installer" "$@"; then
    return 0
  fi
  say "官方安装未完成，正在修复官方 Hermes 源码目录并重试。"
  move_hermes_source_aside
  bash "$installer" "$@"
}
...
ensure_official_hermes() {
  ...
  if [ "$INCLUDE_DESKTOP" = "1" ]; then
    ...
    run_official_installer_with_repair "$official_installer" --include-desktop
  else
    ...
    run_official_installer_with_repair "$official_installer"
  fi
  ...
  if [ "$INCLUDE_DESKTOP" = "1" ] && [ -z "$(find_hermes_desktop)" ]; then
    say "官方 Hermes 桌面端未生成，正在清理官方源码目录后再试一次。"
    move_hermes_source_aside
...
import sys
DATA = """
	
"""
